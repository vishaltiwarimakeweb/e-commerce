...
# Build session: ShopWise eve assistant (see eve-docs/PROGRESS.md) — a deployment fix that touched original app code

Prompt that surfaced this: the user reported a Vercel deploy error — `Edge Runtime is not supported in services. Service "eve" produced Edge Function output "_middleware".` — after deploying with the eve agent wired in via `withEve()`.

- **`src/middleware.ts` removed.** Root cause (traced through eve's own compiled build source, not guessed): eve deploys the chat agent as a co-located Vercel "service" alongside the Next.js app, and its build step unconditionally copies the host app's compiled middleware function into that service's output so middleware still applies to routes served by it. Next.js middleware always compiles to an Edge Function (no `runtime: "nodejs"` option exists for it in this Next.js version), and Vercel's "services" primitive rejects Edge Function output outright — hence the error. There's no config flag in eve to opt out of that copy step. Before removing the file, checked every route in its matcher (`/profile`, `/cart`, `/orders`, `/checkout`) and confirmed each page component already has its own equivalent `getSessionUser()` + `redirect()` guard (and `/admin`, never covered by the middleware matcher in the first place, was already fully protected the same way) — so the middleware was pure redundant defense-in-depth, not the only thing standing between those routes and an unauthenticated request. Removing it is a deployment fix with zero functional regression, not a security-scope reduction.
- Considered and rejected: deploying the eve agent as a fully separate Vercel project. `EVE_NEXT_PRODUCTION_ORIGIN` (the env var meant for "eve lives elsewhere") only takes effect when `process.env.VERCEL` is unset — on an actual Vercel build that check short-circuits back to the co-located path regardless, so a second project wouldn't have avoided the conflict without also disabling `withEve()`'s co-location and making the widget call the agent cross-origin (CORS). Confirmed this in eve's compiled source before proposing it as an option, not from documentation alone.
